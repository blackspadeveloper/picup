...
import 'package:picup/file/save_file_mobile.dart';
import 'package:syncfusion_flutter_xlsio/xlsio.dart';

Future<void> generateExcel(BuildContext context, String? filePath, Function(bool result) callback) async {
  ...
  if (filePath != null) {
    ...
    File compressedFile = await FlutterNativeImage.compressImage(file.path,
        quality: 60,
        percentage: 60,
        targetWidth: decodedImage.width ~/ 8,
        targetHeight: decodedImage.height ~/ 8);
    final List<int> fileBytes = compressedFile.readAsBytesSync();
    final Picture picture = sheet.pictures.addStream(30, 1, fileBytes);
  }
  ...
  debugPrint('formatDate : $formatDate');

  //Save and launch the file.
  await saveAndLaunchFile(bytes, 'pic_up_$formatDate.xlsx', context, (result) {
    callback(result);
  });
}