...
class TextDetectorV2 {
  ...
  Future<RecognisedText> processImage(InputImage inputImage,
    ...
    //       text = text.substring(0,startIdx)+text.substring(endIdx+1,text.length);
    //     }
    //     block['text'] = text;
    //     // log('processImage block :: ${block['text']}');
    //     // log('=========================');
    //   }
    // }
    final recognisedText = RecognisedText.fromMap(result);
...
class RecognisedText {
  ...
  factory RecognisedText.fromMap(Map<dynamic, dynamic> map) {
    ...
    for (var block in map["blocks"]) {
      ...
      if(start >= 0 && end > 0) {
        textBlock.text = text.replaceAll(text.substring(start, end+1), '').trim();
      }
      // add
      // log('textBlock text: ${textBlock.text.trim()} ....');
